...
namespace zonetool::t7
{
	...
	struct FxElemDef
	{
		...
		byte inhIntervalCount;
		byte attIntervalCount;
		...
		byte displacement;
		byte lightingFrac;
		byte depthMultiplier;
		byte pad_1f3[1];
		unsigned short alphaFadeTimeMsec;
		unsigned short maxWindStrength;
		unsigned short spawnIntervalAtMaxWind;
		unsigned short lifespanAtMaxWind;
		FxElemDefUnion u;
		byte pad_204[4];
		...
		float alphaDissolve;
		float zFeather;
		unsigned int falloffBeginAngle;
		unsigned int falloffEndAngle;
		float hdrScale;
		...
		bool useOldHDRScale;
		byte pad_259[1];
		unsigned short libDefIndex;
		unsigned short libVisualIndex;
		byte pad_25e[2];
	};
	...
	static_assert(offsetof(FxElemDef, elemType) == 0xC8);
	static_assert(offsetof(FxElemDef, velSamples) == 0xD0);
	static_assert(offsetof(FxElemDef, visuals) == 0xF0);
	static_assert(offsetof(FxElemDef, u) == 0x1FC);
	static_assert(offsetof(FxElemDef, extended) == 0x1E8);
	...
	struct FxEffectDef
	{
		...
		byte normalsShape;
		byte pad_69[3];
		float normalsOffset[3];
		float normalsNormal[3];
		int gpuMsecLife;
		unsigned short libDefIndex;
		byte pad_8a[14];
	};
	static_assert(sizeof(FxEffectDef) == 0x98);
	static_assert(offsetof(FxEffectDef, elemDefCountLooping) == 0x0C);
	static_assert(offsetof(FxEffectDef, totalSize) == 0x14);
	static_assert(offsetof(FxEffectDef, elemDefs) == 0x20);
	static_assert(offsetof(FxEffectDef, boundingBoxDim) == 0x28);
	static_assert(offsetof(FxEffectDef, libDefIndex) == 0x88);
	...
	static_assert(sizeof(XAssetEntry) == 32);
}
